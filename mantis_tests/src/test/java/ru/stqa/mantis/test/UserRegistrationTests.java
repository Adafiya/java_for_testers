...
public class UserRegistrationTests extends TestBase {
  ...
  void canRegisterUser(String username) { //username создается рандомно и подтягивается из randomUser
    ...
    //Создаем пользователя (адрес) на почтовом сервере (JamesApi)
...
/*
  ...
  @ParameterizedTest
  @MethodSource("randomUser")
  void canRegisterUser1(String username) { //username создается рандомно и подтягивается из randomUser
    //Создаем email
    var email = String.format("%s@localhost", username);
    var password = "password";
